...
parser.add_argument('-alt','--alt', help='Alternate Generation', default=False, action='store_true')
...
for dt in tqdm(data_loader):
    ...
    with torch.no_grad():
        if(alt):
            output = model.lm.generate(input_ids=input_ids, attention_mask=attention_mask, 
                                num_beams=beam, max_length=gmax, min_length=gmin, length_penalty=pen)
        else:
            output = model.lm.generate(input_ids=input_ids, attention_mask=attention_mask, 
                                num_beams=beam, max_new_tokens=gmax, min_new_tokens=gmin, length_penalty=pen)
        
        dec_inp = torch.zeros(size=(input_ids.size(0),1), dtype=input_ids.dtype).to(device)
        lm_out = model.lm(input_ids=input_ids, attention_mask=attention_mask, 
    ...
    predictions += out
    lst_key_tokens+=lst_idx
    lst_prob.extend(lst_val)
...
for i in range(n):
    ...
    s_out = f"{utt.strip()} <keywords> {lst_idx_val}\n"
    f.write(s_out)
f.close()
