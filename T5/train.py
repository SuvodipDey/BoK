...
import transformers
from transformers import AutoTokenizer, T5ForConditionalGeneration, TrainingArguments, Trainer, default_data_collator, EarlyStoppingCallback
...
parser.add_argument('-epochs','--epochs', help='Number of epochs', type=int, required=False, default=20)
...
tokenizer.add_tokens([new_tokens['eou']], special_tokens=True)

if(dataset=="pc"):
    tokenizer.add_tokens([new_tokens['knlg']], special_tokens=True)
...
def prepare_features(df):
    ...
    output = tokenizer(trm_input, max_length=max_len, truncation=True, padding = 'max_length')
        
    tokenized_response = tokenizer(text_target = response, 
                                       max_length=max_len_response, truncation=True)
    label = tokenized_response["input_ids"]
    label.extend([-100]*(max_len_response-len(label)))
    output["labels"] = label
    
    if(use_key):
        tokenized_keys = tokenizer(text_target = df["key_words"])
        tok_key = tokenized_keys["input_ids"][:-1]
        n = len(tok_key)
        if (n>max_bow):
...
logger.info(f"input_ids: {tokenizer.decode(tokenized_datasets['test']['input_ids'][idx], skip_special_tokens=False).replace(tokenizer.pad_token,'')}")
lbl = tokenized_datasets["test"]["labels"][idx]
lbl = [v if v!=-100 else tokenizer.pad_token_id for v in lbl]
logger.info(f"labels: {tokenizer.decode(lbl, skip_special_tokens=False).replace(tokenizer.pad_token,'')}")
if(all_key):
    logger.info(f"labels ids: {tokenized_datasets['test']['labels'][idx]}")
if(use_key):
    key_lbl = tokenized_datasets['test']['key_ids'][idx]
    key_lbl = [v if v!=-100 else tokenizer.pad_token_id for v in key_lbl]
...
logger.info(f"input_ids: {tokenizer.decode(tokenized_datasets['test']['input_ids'][idx+1], skip_special_tokens=False).replace(tokenizer.pad_token,'')}")
lbl = tokenized_datasets["test"]["labels"][idx+1]
lbl = [v if v!=-100 else tokenizer.pad_token_id for v in lbl]
logger.info(f"labels: {tokenizer.decode(lbl, skip_special_tokens=False).replace(tokenizer.pad_token,'')}")
if(all_key):
    logger.info(f"labels ids: {tokenized_datasets['test']['labels'][idx+1]}")
if(use_key):
    key_lbl = tokenized_datasets['test']['key_ids'][idx+1]
    key_lbl = [v if v!=-100 else tokenizer.pad_token_id for v in key_lbl]
    logger.info(f"key_ids: {tokenizer.decode(key_lbl, skip_special_tokens=False)}")
    logger.info(f"key_ids: {tokenized_datasets['test']['key_ids'][idx+1]}")
logger.info("-"*30)

logger.info(f"input_ids: {tokenizer.decode(tokenized_datasets['test']['input_ids'][idx+2], skip_special_tokens=False).replace(tokenizer.pad_token,'')}")
lbl = tokenized_datasets["test"]["labels"][idx+2]
lbl = [v if v!=-100 else tokenizer.pad_token_id for v in lbl]
logger.info(f"labels: {tokenizer.decode(lbl, skip_special_tokens=False).replace(tokenizer.pad_token,'')}")
if(use_key):
    key_lbl = tokenized_datasets['test']['key_ids'][idx+2]
    key_lbl = [v if v!=-100 else tokenizer.pad_token_id for v in key_lbl]
    logger.info(f"key_ids: {tokenizer.decode(key_lbl, skip_special_tokens=False)}")
    logger.info(f"key_ids: {tokenized_datasets['test']['key_ids'][idx+2]}")
logger.info("-"*30)

logger.info(f"input_ids: {tokenizer.decode(tokenized_datasets['test']['input_ids'][idx+3], skip_special_tokens=False).replace(tokenizer.pad_token,'')}")
lbl = tokenized_datasets["test"]["labels"][idx+3]
lbl = [v if v!=-100 else tokenizer.pad_token_id for v in lbl]
logger.info(f"labels: {tokenizer.decode(lbl, skip_special_tokens=False).replace(tokenizer.pad_token,'')}")
if(use_key):
    key_lbl = tokenized_datasets['test']['key_ids'][idx+3]
    key_lbl = [v if v!=-100 else tokenizer.pad_token_id for v in key_lbl]
    logger.info(f"key_ids: {tokenizer.decode(key_lbl, skip_special_tokens=False)}")
    logger.info(f"key_ids: {tokenized_datasets['test']['key_ids'][idx+3]}")
logger.info("-"*30)

logger.info(f"input_ids: {tokenizer.decode(tokenized_datasets['test']['input_ids'][idx+4], skip_special_tokens=False).replace(tokenizer.pad_token,'')}")
lbl = tokenized_datasets["test"]["labels"][idx+4]
lbl = [v if v!=-100 else tokenizer.pad_token_id for v in lbl]
logger.info(f"labels: {tokenizer.decode(lbl, skip_special_tokens=False).replace(tokenizer.pad_token,'')}")
if(use_key):
    key_lbl = tokenized_datasets['test']['key_ids'][idx+4]
    key_lbl = [v if v!=-100 else tokenizer.pad_token_id for v in key_lbl]
    logger.info(f"key_ids: {tokenizer.decode(key_lbl, skip_special_tokens=False)}")
    logger.info(f"key_ids: {tokenized_datasets['test']['key_ids'][idx+4]}")
logger.info("-"*30)
logger.info("-"*30)
...
def model_init():
    model = Model(MODEL_CKPT, tokenizer, use_key, hidden_size)
    return model
