...
raw_model = Model(MODEL_CKPT, tokenizer, use_key, hidden_size, rep_token_id)
raw_model.to(device)
...
for dt in tqdm(data_loader):
    ...
    with torch.no_grad():
        output = model.generate(input_ids=input_ids, attention_mask=attention_mask, num_beams=beam, 
...
for line in predictions:
    resp = line.split(new_tokens['rep'])[1]
